package com.example.dmitryproject.config;


//4.создаем конфигурационный файл спринг секьюрити, принимаем сервисный слой поиска пользователя по логину PersonDetailsService, СС будет сам работать с паролем

import com.example.dmitryproject.services.PersonDetailsService;
...
public class SecurityConfig {
    ...
    //подключаем шифрование бикрипт
    @Bean
    public PasswordEncoder getPasswordEncode() {
        return new BCryptPasswordEncoder();
    }

    // HttpSecurity http - объект аутентификации?
    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {

        http
                .authorizeHttpRequests() // указываем что все страницы должны быть защищены аутентификацией
                .requestMatchers("/admin", "/admin/editTour").hasRole("ADMIN") // указываем на то что страница /admin доступна пользователю с ролью ADMIN
                ...
                .defaultSuccessUrl("/user", true) // Указываем на какой url необходимо направить пользователя после успешной аутентификации. Вторым аргументом указывается true чтобы перенаправление шло в любом случае послу успешной аутентификации
    ...
    //указывает что любая аутентификация должна осуществляться с помощью данного метода
